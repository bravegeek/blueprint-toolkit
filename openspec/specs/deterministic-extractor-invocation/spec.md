# deterministic-extractor-invocation

## Purpose

Defines how the assessment skill's Extraction step invokes deterministic and framework extractors (`tsserver-extractor.mjs`, `nextjs-recipe-extractor.mjs`) alongside `llm-assessment`, ensuring available sources are unioned rather than silently degraded to LLM-only, and that the invocation instructions stay in sync with the scripts on disk.
