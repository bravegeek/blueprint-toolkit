...
Defines a single-command remote bootstrap entry point (`curl -fsSL <url> | bash -s -- init <target>`) that fetches the toolkit and runs `init`, so a new user can onboard without first cloning the repository, and without the bootstrap introducing any dependency beyond what the toolkit already requires.
